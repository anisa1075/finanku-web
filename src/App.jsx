...
export default function App() {
  const messages = ["Selamat Datang", "Di", "Web Finanku"];
  ...
  // kalau mau tutup setelah klik menu
const closeMenu = () => setClick(false);
  const content = <>
    <div className='lg:hidden absolute top-16 w-screen h-screen left-0 right-0 bg-white cursor-pointer  flex flex-col items-center justify-center transform transition-all duration-500 ease-in-out'>
      <ul className='text-center text-[#16294A] font-outfit  font-medium text-2xl transition hover:border-[#67C9CB] cursor-pointer'>
        <Link spy={true} to="/" smooth={true} duration={1000} onClick={closeMenu}><li className=' my-4 py-4 hover:rounded'>Home</li></Link>
        <Link spy={true} to="/article" smooth={true} duration={1000} onClick={closeMenu}><li className=' my-4 py-4 hover:rounded'>Article</li></Link>
        <Link spy={true} to="/budget" smooth={true} duration={1000} onClick={closeMenu}><li className=' my-4 py-4 hover:rounded'>Get Started</li></Link>
      </ul>
    </div>
  </>
  ...
  return (
    <div className=''>
        ...
        <div className='flex justify-between z-50 text-white lg:py-3 lg:px-20 px-10 py-4'>
          ...
          <div className='lg:flex md:flex lg:flex-1 items-center font-normal'>
            <div className='flex-10'>
              <ul className='hidden lg:flex md:flex gap-8 mr-16 text-[16px] font-outfit items-center w-full justify-end'>
                ...
                <Link spy={true} to="/budget" smooth={true} duration={1000}>
                  <li >
                    <button className='btn liquid bg-transparent font-medium text-base border-1 lg:py-3 md:py-2 border-[#9B51E0] text-[#9B51E0] px-6 rounded-full tracking-wide cursor-pointer'>Get Started</button>
                  </li>
                </Link>
              </ul>
            </div>
      ...
      <img className=' absolute top-0 z-0' src={Bg1} alt="Money" />
